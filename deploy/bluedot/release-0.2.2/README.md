# myextension 0.2.2 BLUEDOT 镜像交付包

本目录是 2026-08-07 演示热修复的独立交付包。0.2.2 在显示知识点、点击“确认知识点”和生成发布草稿三个边界自动补齐缺失或损坏的“过程观察问题 / 支持表现 / 排除情况”，保留老师已经填写的有效内容；只有知识点名称本身缺失时才继续要求人工填写。它同时包含此前的分析可靠性、左侧“行为分析”标签方向和测试建议延迟修复。

它安装 JupyterLab 4 的 prebuilt 前端扩展和 Jupyter Server 2 后端扩展，不需要在目标镜像中安装 Node.js，也不会修改基础镜像原有的 `ENTRYPOINT` 或 `CMD`。
...
| 文件 | 用途 |
| --- | --- |
| `artifacts/myextension-0.2.2-py3-none-any.whl` | 修复后的插件 wheel |
...
插件已提升为唯一版本 `0.2.2`，不要再以同版本覆盖旧 0.2.1 wheel：

```text
d204b91e1724cea9d4870312cb3eb46c3e4a9afbc4de92434054fd888d95affc  artifacts/myextension-0.2.2-py3-none-any.whl
```
...
```bash
cd deploy/bluedot/release-0.2.2
```
...
必须显示 `artifacts/myextension-0.2.2-py3-none-any.whl: OK`。不一致时停止，不要构建或安装。
...
python -m pip install \
  ...
  --no-deps \
  --force-reinstall \
  artifacts/myextension-0.2.2-py3-none-any.whl
...
```bash
python -c "import myextension; assert myextension.__version__ == '0.2.2'; print(myextension.__version__)"
...
安装完成后关闭所有旧 JupyterLab 标签页，用全新标签打开工作台并强制刷新一次。浏览器开发者工具的 Network 中应加载 `remoteEntry.9a2b667cad0b061a.js`；如果仍加载 0.2.1 的 `remoteEntry.80d52557487e4002.js`，说明镜像 digest、工作台容器或浏览器缓存仍是旧版本。

临时 PyPI Manager 安装只适合验证，工作台重建后可能丢失，正式交付应使用镜像。
...
TARGET_IMAGE='<单位镜像仓库>/bluedot-jupyterlab4-behavior-audit:0.2.2-demo-hotfix-20260807'
```
...
推送后记录新镜像 digest，并用该 digest 创建一个全新的工作台验证。不能只重启旧容器，因为旧工作台可能仍固定到旧镜像 digest。

## 9. 0.2.2 最新运行配置逻辑

### 知识点和测试建议请求

- 采用 AI 建议后，缺失、空白或类型异常的三个观察字段会自动使用知识点名称生成可演示的默认文案；
- 自动补全在界面显示、知识点确认和发布草稿构建三个边界执行，旧草稿也能自愈；
- 已有的非空老师文案只会去除首尾空白，不会被默认值覆盖；
