...
class StudentBindingV1:
    """The canonical V1 identity snapshot embedded in a child project description."""
...
def encode_student_binding_v1(binding: StudentBindingV1) -> str:
    ...
    canonical_json = _canonical_json(binding)
    try:
        raw_json = canonical_json.encode("utf-8")
...
def parse_student_binding_description(description: str) -> StudentBindingV1 | None:
    ...
    if v1_match is None:
        unknown_version_match = _UNKNOWN_VERSION_FIRST_LINE_PATTERN.fullmatch(first_line)
        if unknown_version_match is not None and unknown_version_match.group(1) != "V1":
...
def parse_legacy_child_name(name: str, prefix: str) -> str | None:
    ...
    pattern = re.compile(re.escape(prefix) + r"-([A-Za-z0-9_-]*)-([0-9a-z]{4})")
...
def _decode_binding_payload(payload: str) -> StudentBindingV1:
    try:
        ...
        raw_json = base64.urlsafe_b64decode(padded_payload.encode("ascii"))
        decoded_json = raw_json.decode("utf-8")
        data = json.loads(decoded_json, object_pairs_hook=_object_without_duplicate_keys)
...
def _canonical_json(binding: StudentBindingV1) -> str:
    ...
    if any(
        not isinstance(value, str) or not value or len(value) > MAX_FIELD_LENGTH
