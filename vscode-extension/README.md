...
8. 点击“结束、生成简报并导出”后，课堂简报会立即在编辑区打开，并先请求导出位置；随后
   状态栏与学生端会同步显示 AI 建议和导出进度。AI 未配置、超时或失败时，仍会导出课堂简报和安全的
   `analysis_log.json`。V2 导出还会包含面向教师的 `teacher_brief.md` 与结构化
   `classroom_brief.json`；AI 只做建议，不能改写本地固定的课题实践表现。
